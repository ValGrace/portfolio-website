...
func main() {
	...
	api.HandleFunc("/formproj", getProjects).Methods("GET")
	api.HandleFunc("/", createProject).Methods("POST")
	spa := clientHandler{staticPath: "../../portfolio/build", indexPath: "index.html"}
	client.PathPrefix("/").Handler(spa)
	client.PathPrefix("/projects").Handler(http.StripPrefix("/projects/", spa))
